...
for anchor,filename in [('start','README.md'),('agent','AGENT_HANDOFF.md'),('automation','AUTOMATION_PROMPT.md')]:
    sections.append('<section lang="'+('en' if anchor=='start' else 'ru')+'" id="'+anchor+'">'+md.render((ROOT/filename).read_text())+'</section>')
...
page='<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Trendwatch · video research for your brand</title><style>'+css+'</style></head><body><header>Trendwatch</header><nav><a href="#start">Get started</a><a href="#agent">Operator guide (RU)</a><a href="#automation">Monitoring (RU)</a></nav><main>'+body+'</main></body></html>'
