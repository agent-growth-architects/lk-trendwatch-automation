...
A standalone HTML with local sibling assets is not transferable alone. Before delivery verify links/anchors/media and actual presentation through permitted tools. If browser inspection is blocked, preserve the blocked status, run static checks and do not bypass tool restrictions. Offer suitable archive/PDF/public-source-link packaging within the user's requested scope; public hosting or outbound sending requires authorization. Include an optional short forwarding message and truthful scope sentence for SMM.

Evidence gate: verified date for every claimed fresh case, sources for counters/ratios, exact baseline population, media-review method and uncertainty, source or new-hypothesis label for each idea. Limited cases belong in a clear appendix. A report can be useful without full access, but must not claim complete coverage.
